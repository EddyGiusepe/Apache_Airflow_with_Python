...
default_args = {
    ...
    'email': ['eddychirinos_unac@hotmail.com'],
...
# Colocamos um decorador --> a nossa dag
@dag(schedule_interval=None, start_date=datetime(2022, 11, 6), catchup=False, tags=['example'])
def Housing():
    """
    ...
    @task()
    def start():
        print("iniciando mosso experimento de ML ...")
    ...
    chain(start(), criar_diretorio(), download_housing_file(), extract_housing_file(),
          criar_novos_campos(), tratar_campos_nulos(), aplicar_one_hot_encoding(), normalizar_dados(),
          dividir_dados_treino_teste(),
          [treinar_LinearRegression(), treinar_DecisionTreeRegressor(), treinar_KNeighborsRegressor(),
